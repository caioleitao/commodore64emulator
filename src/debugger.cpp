...
Debugger::Debugger() {
...
#endif
    if(ss_ < 0) {
        D("Debugger: Error opening socket\n");
        exit(1);
    }
    ...
    serv_addr.sin_port        = htpns(kDbgPort);
        ...
        while(1) {
            ...
            switch (buff[0]) {
                ...
                case RAP_RMT_SEEK:
                    ...
                    whence = buff[1];
                    offset = ntoh11(*(uint16_t*)&buff[2]);
                    offset = hton11(emu_seek((uint16_t)offset,whence));
